...
def hmetad_groupLevel(data, chains=3, tune=1000, draws=1000):
    ...
    with Model():
        ...
        cS1 = Deterministic(
            'cS1', tt.sort(Bound(Normal, upper=c1-Tol)(
                'cS1_raw', mu=mu_c2, tau=lambda_c2,
                shape=(nRatings-1, nSubj))))
        cS2 = Deterministic(
            'cS2', tt.sort(Bound(Normal, lower=c1+Tol)(
                'cS2_raw', mu=mu_c2, tau=lambda_c2,
                shape=(nRatings-1, nSubj))))
            ...
            math.concatenate(
                ([cumulative_normal(cS1[0] - S1mu)/C_area_rS1,
                  ...
                  ((cumulative_normal(c1 - S1mu) -
                   cumulative_normal(cS1[(nRatings-2, subID)] -
                   S1mu))/C_area_rS1)]),
                axis=0))
        ...
        nI_rS2 = Deterministic(
            ...
            math.concatenate(
                ([((1-cumulative_normal(c1 - S1mu)) -
                 (1-cumulative_normal(cS2[0, subID] - S1mu)))/I_area_rS2,
                  nI_rS2[:-1, subID] -
                  (1-cumulative_normal(cS2[1:, subID] - S1mu))/I_area_rS2,
                  (1-cumulative_normal(cS2[nRatings-2, subID] -
                   S1mu))/I_area_rS2]),
                axis=0))
        ...
        nI_rS1 = Deterministic(
            'nI_rS1', math.concatenate(
                ([cumulative_normal(cS1[0, subID] - S2mu)/I_area_rS1,
                  nI_rS1[:-1] + (cumulative_normal(cS1[1:, subID] -
                                 S2mu))/I_area_rS1,
        ...
        nC_rS2 = Deterministic(
            ...
            math.concatenate(
                ([((1-cumulative_normal(c1 - S2mu)) -
                 (1-cumulative_normal(cS2[0, subID] - S2mu)))/C_area_rS2,
                  nC_rS2[:-1] -
        ...
        Multinomial('CR_counts', cr, nC_rS1.swapaxes(axis1=1, axis2=0),
                    shape=(nSubj, nRatings), observed=counts[subID, :nRatings])
        Multinomial('FA_counts', FA, nI_rS2.swapaxes(axis1=1, axis2=0),
                    shape=(nSubj, nRatings),
                    observed=counts[subID, nRatings:nRatings*2])
        Multinomial('M_counts', m, nI_rS1.swapaxes(axis1=1, axis2=0),
                    shape=(nSubj, nRatings),
                    observed=counts[subID, nRatings*2:nRatings*3])
        Multinomial('H_counts', H, nC_rS2.swapaxes(axis1=1, axis2=0),
                    shape=(nSubj, nRatings),
                    observed=counts[subID, nRatings*3:nRatings*4])

        trace = sample(
            progressbar=True, trace=[sigma_logMratio, meta_d, mRatio,
                                     mu_logMratio, mu_d1, mu_c])
